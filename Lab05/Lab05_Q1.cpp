...
public:
    ...
    void enqueue(Card card) {
        ...
        deck.push_back(card); // 將card放入到deck
        if (Front == -1) Front = 0; // 如果是第一次插入，將Front設置為0
        Rear++;
    }
    ...
    void dequeue() {
        ...
        Front++;
        if (Front > Rear) { // 如果Front指標等於Rear，重置Front和Rear，並清空deck
            Front = -1;
            Rear = -1;
            deck.clear();
        }
...
void initializePlayer(Player* player, string name, Queue& cardDeck) { // 初始化玩家，發兩張牌
    ...
    for (int i = 0; i < 2; i++) { // 發兩張牌
        ...
        if (drawnCard.rank == "J" || drawnCard.rank == "Q" || drawnCard.rank == "K") player->score += 10; // 計算JQK點數
...
void playerTurn(Player* player, Queue& cardDeck) { // 玩家回合
    ...
    while (player->score < 21 && !cardDeck.isEmpty()) { // 玩家小於21點且牌堆不為空
        ...
        if (choice == 'h') { // 如果玩家選擇抽牌
            ...
            cardDeck.dequeue(); // 然後從牌堆中移除這張牌
            if (newCard.rank == "A") { // 如果是A，詢問玩家要1點還是10點
                int aceChoice;
                cout << "你抽到A，選擇要1點還是10點 (輸入1或10): ";
                cin >> aceChoice;
...
void dealerTurn(Player* dealer, Queue& cardDeck) { // 莊家回合
    while (dealer->score < 17 && !cardDeck.isEmpty()) { // 莊家小於17點且牌堆不為空
        Card newCard = cardDeck.front(); // 從牌堆中取出一張牌
        cardDeck.dequeue(); // 然後從牌堆中移除這張牌
        if (newCard.rank == "A") { // 如果是A，根據莊家的分數來決定是加1還是加10
            if (dealer->score <= 11) {
                dealer->score += 10; // 如果莊家的分數小於或等於10，A記為11點
            } else {
                dealer->score += 1; // 否則A記為1點
            }
